...
DIFF=$(git -c color.ui=never -c core.pager=cat diff --no-ext-diff -- "$FILE_PATH" 2>/dev/null; git -c color.ui=never -c core.pager=cat diff --cached --no-ext-diff -- "$FILE_PATH" 2>/dev/null)
[ -z "$DIFF" ] && exit 0

# An "assertion change" is a line that *adds or removes* an assert/expect/require.
ASSERT_DIFF=$(printf '%s' "$DIFF" | grep -E '^[+-][^+-].*(assert\.|require\.|expect\(|[[:space:]]assert[[:space:]]|[[:space:]]assert$)' || true)
...
SLICE_DIFF=$(git -c color.ui=never -c core.pager=cat diff --no-ext-diff 2>/dev/null; git -c color.ui=never -c core.pager=cat diff --cached --no-ext-diff 2>/dev/null)
if printf '%s' "$SLICE_DIFF" | grep -qE '^\+.*(TC-[0-9]+|testcase[[:space:]]*[:=]|(Refs|Fixes|Closes)[[:space:]]*#[0-9]+)'; then
  exit 0
fi

# All three exits failed: block. This guard exists to prevent moving assertions
# to get green without production/spec evidence.
log ""
