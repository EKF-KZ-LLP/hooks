...
while IFS= read -r testfile; do
  [ -z "$testfile" ] && continue
  DIFF=$(git -c color.ui=never -c core.pager=cat diff --no-ext-diff -- "$testfile" 2>/dev/null; git -c color.ui=never -c core.pager=cat diff --cached --no-ext-diff -- "$testfile" 2>/dev/null)
  if printf '%s' "$DIFF" | grep -qE '^\+.*(assert\.|require\.|expect\(|[[:space:]]assert[[:space:]]|[[:space:]]assert$)'; then
