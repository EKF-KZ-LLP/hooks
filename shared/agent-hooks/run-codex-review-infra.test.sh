...
scripts=(
  "/Users/antonsahovskii/.claude/hooks/run-codex-review-infra.sh"
  "/Users/antonsahovskii/.claude-personal/hooks/run-codex-review-infra.sh"
  "/Users/antonsahovskii/.claude-work/hooks/run-codex-review-infra.sh"
  "/Users/antonsahovskii/.claude-ekfgroup/hooks/run-codex-review-infra.sh"
)

tmp_root="$(mktemp -d "${TMPDIR:-/tmp}/run-codex-review-infra-test.XXXXXX")"
...
if (mode === "pass_plain") {
  process.stdout.write("Reviewed shell wrapper.\nVerdict: PASS\n");
...
make_repo() {
  ...
  (
    ...
    printf '%s\n' '#!/usr/bin/env bash' 'echo ok' > sample.sh
    git add sample.sh
    git commit -q -m init
  )
}
...
run_case() {
  ...
  (
    ...
    CLAUDE_PLUGIN_ROOT="$plugin" FAKE_CODEX_REVIEW_MODE="$mode" \
      bash "$script" "test-$mode" "$repo/sample.sh"
  ) >"$out" 2>&1
  local status=$?
  set -e
...
echo "run-codex-review-infra tests passed"
